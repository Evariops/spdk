...
| 0041 | lvol → tier band usage provider — when an lvolstore loads on a `bdev_tier` composite the lvol layer registers a per-band fill-accounting provider (unregistered on unload); `bdev_tier_get_bands` then fills `used_blocks` by counting allocated clusters of the blobstore's `used_clusters` pool intersecting the band's LBA range (`spdk_bs_count_allocated_clusters_in_lba_range`, new in lib/blob) | lib/blob, bdev_lvol; `#include`s `vbdev_tier.h` (requires the tier module, like 0005) | 0004 (bit-pool accessors), 0005 (lvol `-I` tier CFLAGS), tier module |
